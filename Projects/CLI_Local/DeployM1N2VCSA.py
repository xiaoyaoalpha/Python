...
# Copy JSON replacement to Jenkins slaves "FilesTempLocation"
jsonfile_replace_vc01 = FileOperation.File(os.path.join(JSONFILE_REPLACE_INSTALL_DIR, JSONFILE_NAME_PSC))
jsonfile_replace_vc01.copy(SLAVE_FILE_TEMP_LOCATION)
jsonfile_target_vc01 = FileOperation.File(os.path.join(SLAVE_FILE_TEMP_LOCATION, JSONFILE_NAME_PSC))
jsonfile_target_vc01.rename(pscName + ".json")
jsonfile_target_vc01 = FileOperation.File(os.path.join(SLAVE_FILE_TEMP_LOCATION, pscName + ".json"))
...
JsonReplace_Dict["new_vcsa"]["esxi"]["deployment_network"] = TargetESXi_networkname
JsonReplace_Dict["new_vcsa"]["esxi"]["datastore"] = TargetESXi_datastorename
JsonReplace_Dict["new_vcsa"]["appliance"]["name"] = vcName01
with open(jsonfile_target_vc01.fullpath, 'w', encoding='UTF-8') as w:
    json.dump(JsonReplace_Dict, w, indent=4)
print("Target ESXi infomation have been writen to Json file!", end="\n")

# Input deploy command in Windows console to deploy VC
cli_cmd_install_M1N2_vCSA_on_ESXi_vc = CLI_DEPLOY_EXE_FULLPATH + r" install --accept-eula --acknowledge-ceip " \
    + jsonfile_target_vc01.fullpath + r" -v --no-ssl-certificate-verification"
print("Deploying VC01 is in progress, please wait...", end="\n")
cli_deploy_result_list_vc01 = list(os.popen(cli_cmd_install_M1N2_vCSA_on_ESXi_vc))
...
# Copy JSON replacement to Jenkins slaves "FilesTempLocation"
jsonfile_replace_vc02 = FileOperation.File(os.path.join(JSONFILE_REPLACE_INSTALL_DIR, JSONFILE_NAME_PSC))
jsonfile_replace_vc02.copy(SLAVE_FILE_TEMP_LOCATION)
jsonfile_target_vc02 = FileOperation.File(os.path.join(SLAVE_FILE_TEMP_LOCATION, JSONFILE_NAME_PSC))
jsonfile_target_vc02.rename(pscName + ".json")
jsonfile_target_vc02 = FileOperation.File(os.path.join(SLAVE_FILE_TEMP_LOCATION, pscName + ".json"))
...
JsonReplace_Dict["new_vcsa"]["esxi"]["deployment_network"] = TargetESXi_networkname
JsonReplace_Dict["new_vcsa"]["esxi"]["datastore"] = TargetESXi_datastorename
JsonReplace_Dict["new_vcsa"]["appliance"]["name"] = vcName02
with open(jsonfile_target_vc02.fullpath, 'w', encoding='UTF-8') as w:
    json.dump(JsonReplace_Dict, w, indent=4)

# Input deploy command in Windows console to deploy VC
cli_cmd_install_M1N2_vCSA_on_ESXi_vc = CLI_DEPLOY_EXE_FULLPATH + r" install --accept-eula --acknowledge-ceip " \
    + jsonfile_target_vc02.fullpath + r" -v --no-ssl-certificate-verification"
print("Deploying VC02 is in progress, please wait...", end="\n")
cli_deploy_result_list_vc02 = list(os.popen(cli_cmd_install_M1N2_vCSA_on_ESXi_vc))
...
jsonfile_target_vc01.remove()
jsonfile_target_vc02.remove()
print('"' + jsonfile_target_psc.fullpath + '"' + " is deleted!", end="\n")
print('"' + jsonfile_target_vc01.fullpath + '"' + " is deleted!", end="\n")
print('"' + jsonfile_target_vc02.fullpath + '"' + " is deleted!", end="\n")
print("==================================================================", end="\n")
